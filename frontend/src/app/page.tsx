...
export default function Home() {
  ...
  const [activeTool, setActiveTool] = useState<"encoding" | "analysis">("encoding");
  ...
  const handleTestFill = async () => {
    // Upload the test CSV — don't block form fill if backend is down
    try {
      const res = await fetch("/test_messages.csv");
      if (!res.ok) throw new Error("csv_missing");
      ...
      try {
        ...
        setUploadError("Backend not reachable — start the server, then re-upload the file.");
      }
      setUploading(false);
    } catch {
      setUploadError("Could not load test CSV. Make sure test_messages.csv is in the public folder.");
    }

    // Set message column (always, even if upload failed)
...
If the message field is empty/blank, classify as: Promise=0, Empty_Talk=0, No_Message=1.`);
    ...
    // Set model slots
    setModelSlots([{ provider: "openai", model: "gpt-4.1-nano", apiKey: "" }]);
    setRunsPerModel(1);
    setAggregation("mode");

    // Open all panels (skip animation for batch open)
    openAllPanels();
  };
    ...
    <>
      <nav className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="btn btn-ghost btn-xs text-muted" onClick={handleTestFill}>Load demo</button>
