...
const FAQS: { q: string; a: React.ReactNode }[] = [
  ...
  },
  {
    ...
    a: <>Use the single results button to download the complete output. With one model call, CAT returns the coded source-row CSV with every variable unchanged. With repeated or multi-model coding, CAT returns one ZIP containing overall results, the corresponding files for each LLM, and every unchanged individual model/run result. When two or more models are used, repeated runs are first aggregated within each model; CAT then reports pairwise exact agreement and unweighted Cohen&apos;s kappa separately for every non-text numeric result column and includes the same inter-coder table as a CSV in the ZIP. Statistics use episodes with values from both models, and kappa is N/A when expected agreement is 100%. After a selective re-run, the replacement calls take the place of the earlier records for those episodes while all unaffected records remain available.</>,
  },
  {
